...
import { Lock, Mail, User as UserIcon, ArrowRight, Loader2, LayoutDashboard, KanbanSquare, Briefcase as BriefcaseIcon, CheckSquare, Key, ShieldAlert, X, Cpu, BarChart3, Zap, CalendarClock, GitGraph, ArrowLeft } from 'lucide-react';
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile, setPersistence, browserLocalPersistence, browserSessionPersistence, GoogleAuthProvider, signInWithPopup, sendEmailVerification, sendPasswordResetEmail } from 'firebase/auth';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLoginSuccess, onSeedingStart, onSeedingEnd }) => {
  ...
  // Input Focus State for Tooltips
  const [focusedField, setFocusedField] = useState<'email' | 'password' | 'username' | 'jobTitle' | null>(null);
  ...
  const [activeFeature, setActiveFeature] = useState<{ 
      ...
      detail: string, 
      style: { border: string, text: string, gradient: string, shadow: string } 
  } | null>(null);
  ...
  // --- Feature Configuration (Clustered 6-Card Layout for Focus Effect) ---
  const features = [
      { 
          id: 'core', 
          icon: Cpu, 
          title: "Core Engine", 
          desc: "Logic-driven Processing", 
          detail: "Precise data processing and rapid execution. Optimize schedules and budget allocation based on strict logic.",
          style: {
      ...
      { 
          ...
          icon: Zap, 
          title: "Real-time", 
          desc: "Synchronized Pulse", 
          detail: "Every change is instantly updated across the system. Your team operates as a single unified entity.", 
          style: {
              border: "border-purple-500",
              text: "text-purple-400",
              gradient: "from-purple-400 to-fuchsia-500",
              shadow: "hover:shadow-[0_0_30px_rgba(192,132,252,0.4)]"
          },
      ...
      { 
          ...
          title: "Project Map", 
          desc: "Mind Mapping", 
          detail: "A holistic view of task relationships. Visualize dependencies, critical paths, and logical structures clearly.", 
          style: {
      ...
      { 
          ...
          title: "Analytics", 
          desc: "Data Vision", 
          detail: "Transform raw numbers into actionable strategies. Monitor cash flow, performance, and progress in real-time.", 
          style: {
      ...
      { 
          ...
          title: "Timeline", 
          desc: "Time Mastery", 
          detail: "Long-term planning with Gantt charts. Identify milestones and manage dependencies effectively.", 
          style: {
      ...
      { 
          ...
          icon: KanbanSquare, 
          title: "Kanban", 
          desc: "Workflow Stream", 
          detail: "Visualize processes with flexible Kanban boards. Eliminate production bottlenecks efficiently.", 
          style: {
  ...
  // Custom cursor SVG for hover effect
  const viewCursor = `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="%23ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>') 16 16, auto`;
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
      ...
      if (adminPasscode === 'tam123456') {
          ...
          // Keeping the exact Vietnamese error message as historically requested for the backdoor
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (authView === 'FORGOT_PASS') return;
  ...
  const InputTooltip = ({ text }: { text: string }) => (
    <>
        {/* Desktop: Right Side */}
        <div className="hidden lg:block absolute left-full top-1/2 -translate-y-1/2 ml-4 w-48 p-3 bg-slate-800 text-white text-xs rounded-xl shadow-xl border border-slate-700 animate-fade-in z-20 pointer-events-none">
            <div className="absolute top-1/2 -left-2 -translate-y-1/2 border-8 border-transparent border-r-slate-800"></div>
            {text}
        </div>
        {/* Mobile: Top */}
        <div className="lg:hidden absolute bottom-full left-0 mb-2 w-full p-2 bg-slate-800 text-white text-xs rounded-lg shadow-xl z-20 pointer-events-none animate-fade-in">
            {text}
  ...
  return (
    <div className="min-h-screen flex bg-white dark:bg-slate-950 font-sans">
      ...
      <style>{`
        @keyframes float {
          0% { transform: translate3d(0, 0, var(--tz)); }
          50% { transform: translate3d(0, -8px, var(--tz)); }
          100% { transform: translate3d(0, 0, var(--tz)); }
        }
        .animate-float {
      ...
      {/* LEFT SIDE - INTERACTIVE 3D STAGE (Hidden on Mobile) */}
      <div 
      ...
      >
         ...
         <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 pointer-events-none"></div>
         <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle, rgba(255,255,255,0.1) 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
         
         {/* Brand Top Left */}
         <div className="relative z-20 p-12 flex items-center gap-4 pointer-events-none">
            <div className="relative">
                <div className="absolute inset-0 bg-blue-500/30 blur-xl rounded-full"></div>
                <div className="relative w-12 h-12 bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/20 shadow-inner">
                    <span className="font-black text-2xl text-white">P</span>
                </div>
            </div>
            <h1 className="font-black text-4xl tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400 drop-shadow-lg">
                ProManage AI
            </h1>
         </div>

         {/* 3D Content Stage */}
         <div className="flex-1 relative flex items-center justify-center perspective-[1500px] z-10 group/showcase">
            <div 
               className="relative w-[700px] h-[500px] transition-transform duration-100 ease-out preserve-3d"
               style={{ transform: `rotateY(${tilt.x * 8}deg) rotateX(${-tilt.y * 8}deg)` }}
            >
                {/* Main Hero Card (Floating Dashboard) - Center - Z-10 */}
                <div className="absolute inset-0 bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 p-6 overflow-hidden transform translate-z-0 z-10 pointer-events-none transition-all duration-500 group-hover/showcase:blur-[2px] group-hover/showcase:scale-95 group-hover/showcase:opacity-60">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="col-span-3 h-24 bg-white/5 rounded-xl border border-white/5 mt-4"></div>
                    </div>
                </div>

                {/* Floating Feature Cards - Z-50 for Clickability */}
                {features.map((f, i) => (
                    <button 
                        key={f.id}
                        onClick={(e) => { e.stopPropagation(); setActiveFeature(f); }}
                        className={`
                            absolute w-64 p-4 rounded-xl bg-slate-900/80 backdrop-blur-xl border border-white/10 shadow-2xl 
                            transition-all duration-500 ease-out 
                            flex items-center gap-4 outline-none 
                            border-l-4 ${f.style.border}
                            ${f.position} backface-hidden transform-gpu antialiased
                            pointer-events-auto z-30
                            group-hover/showcase:opacity-40 group-hover/showcase:blur-[2px] group-hover/showcase:scale-95 group-hover/showcase:grayscale-[0.5]
                            hover:!opacity-100 hover:!blur-0 hover:!scale-110 hover:!grayscale-0 hover:!z-50
                            ${f.style.shadow} hover:bg-slate-800
                            animate-float
                        `}
                        style={{ 
                            zIndex: 50 + i, // Ensure cards are well above the hero image
                            cursor: viewCursor,
                            '--tz': '40px',
                            animationDelay: `${i * 0.7}s` 
                        } as React.CSSProperties}
                    >
                        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${f.style.gradient} bg-opacity-20 flex items-center justify-center text-white shadow-lg shrink-0`}>
                            <f.icon size={20} />
                        </div>
                        <div className="text-left min-w-0">
                            <p className={`text-xs font-bold uppercase tracking-wider mb-0.5 ${f.style.text}`}>{f.title}</p>
                            <p className="text-xs font-medium text-slate-300 leading-tight line-clamp-2">{f.desc}</p>
                        </div>
                    </button>
                ))}
            </div>
         ...
         {/* Text Block */}
         <div className="relative z-20 p-12 pb-16 pointer-events-none">
             <h1 className="text-5xl font-bold text-white mb-4 leading-tight">
                The Future of <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">Project Management</span>
             </h1>
             <p className="text-slate-400 max-w-lg text-lg">Experience the synergy of Logic-driven management and real-time collaboration.</p>
         </div>
      ...
      <div className="w-full lg:w-[40%] bg-white dark:bg-slate-950 flex flex-col justify-center items-center p-6 md:p-12 relative transition-all duration-500">
         ...
         <div className="w-full max-w-[400px] animate-fade-in relative z-10">
            ...
            <div className="mb-10">
               ...
               {authView === 'FORGOT_PASS' ? (
                   ...
                   <>
                       <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">{authView === 'LOGIN' ? 'Welcome Back!' : 'Join the Future'}</h2>
                       <p className="text-slate-500 dark:text-slate-400">{authView === 'LOGIN' ? 'Sign in to access your smart workspace.' : 'Start your 14-day free trial today.'}</p>
                   </>
               )}
            </div>
            ...
            {authView === 'FORGOT_PASS' ? (
                ...
                <form onSubmit={handleResetPassword} className="space-y-5">
                    ...
                    <button 
                        ...
                        className="w-full py-2 text-sm font-bold text-slate-500 hover:text-indigo-600 flex items-center justify-center gap-2 transition-colors"
                    >
                ...
                <>
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative group">
                                ...
                                <input 
                                    ...
                                    required
                                />
                                {focusedField === 'email' && <InputTooltip text="Please enter your registered work or personal email." />}
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            <div className="flex justify-between items-center">
                                ...
                                {authView === 'LOGIN' && (
                                    <button 
                                        type="button"
                                        onClick={() => setAuthView('FORGOT_PASS')}
                                        className="text-xs font-bold text-indigo-600 hover:underline hover:text-indigo-500"
                                    >
                                        Forgot Password?
                                    </button>
                                )}
                            ...
                            <div className="relative group">
                                ...
                                <input 
                                    ...
                                    required
                                />
                                {focusedField === 'password' && <InputTooltip text="Enter your secure password. Case sensitive." />}
                            </div>
                        </div>
                        ...
                        className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                        >
                        {isLoading ? <Loader2 className="animate-spin" size={20} /> : <>{authView === 'LOGIN' ? 'Sign In' : 'Create Account'} <ArrowRight size={20} /></>}
                        </button>
                    </form>
                    ...
                    <div className="mt-8 text-center">
                        ...
                        <button onClick={() => setAuthView(authView === 'LOGIN' ? 'REGISTER' : 'LOGIN')} className="ml-1.5 text-indigo-600 dark:text-indigo-400 font-bold hover:underline">
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/90 backdrop-blur-lg animate-fade-in p-6">
              <div className="relative w-full max-w-4xl bg-gradient-to-br from-slate-900 to-indigo-950 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
                  ...
                  <div className="w-full md:w-1/2 p-12 flex items-center justify-center bg-black/20 relative overflow-hidden">
                      <div className={`absolute inset-0 bg-gradient-to-br ${activeFeature.style.gradient} opacity-20 mix-blend-overlay`}></div>
                      <div className={`relative z-10 w-48 h-48 rounded-full bg-white/5 animate-pulse flex items-center justify-center border-2 ${activeFeature.style.border.replace('border', 'border-opacity-50')}`}>
                          <activeFeature.icon size={80} className={activeFeature.style.text} />
                      </div>
                  </div>

                  {/* Content Side */}
                  <div className="w-full md:w-1/2 p-12 flex flex-col justify-center">
                      <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${activeFeature.style.gradient} flex items-center justify-center text-white mb-6 shadow-lg`}>
                          <activeFeature.icon size={24} />
                      </div>
                      <h3 className="text-3xl font-bold text-white mb-4">{activeFeature.title}</h3>
                      <p className={`text-xl mb-6 font-light italic ${activeFeature.style.text}`}>"{activeFeature.desc}"</p>
